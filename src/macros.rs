    ...
    ($map_access:ident key $expecting:ident) => {
        {
            ...
            match __key {
                ...
                None => return Err(<V::Error as ::serde::de::Error>::custom(format!("Expected key {} to exist", $expecting)))
            }
        }
    };
...
macro_rules! unexpected_key_access {
    ($key:ident) => {
        Err(<V::Error as ::serde::de::Error>::custom(format!(
            "Expected STRUCTURE_SIG_KEY. Got {} instead.",
            $key
        )))
    };
    () => {
        Err(<V::Error as ::serde::de::Error>::custom(
